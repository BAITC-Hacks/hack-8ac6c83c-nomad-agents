...
- Keep all application data inside the singleton store. `demo`, `full`, and reset operations clear and repopulate these collections. A process restart intentionally returns the application to an empty state until seed is called.

## Acceptance

`dotnet build api/TaskForge.Api.csproj` passes. In one API process, manually write and read a business, team, draft task, confirmed task, rating, proposal, and AI log; compare key fields. Verify the published query returns only published tasks, concurrent stale revision is rejected, reset empties every collection, and restarting the API clears all state.
